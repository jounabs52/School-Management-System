    ...
    <div className="p-1.5 sm:p-2 md:p-3 lg:p-4">
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2">
        {toasts.map(toast => (
          <div
            key={toast.id}
            className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg shadow-lg bg-green-600 text-white min-w-[250px] sm:min-w-[300px]"
          >
            {toast.type === 'success' && <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5" />}
            {toast.type === 'error' && <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-red-400" />}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-3 mb-4">
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
            <button
              ...
              className="flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white py-1.5 sm:py-2 px-3 rounded-lg text-xs sm:text-sm transition"
            >
        ...
        {activeTab === 'list' && (
          <div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 mb-4">
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <input
                  ...
                  className="flex-1 border border-gray-300 rounded px-3 py-2 sm:py-2.5 text-sm sm:text-base w-full"
                />
                <button className="bg-red-500 hover:bg-red-600 text-white py-1.5 sm:py-2 px-3 rounded-lg text-xs sm:text-sm whitespace-nowrap">
                  Search
                </button>
              </div>
            </div>
            ...
            {filterClass && filterSubjects.length > 0 && (
              <div className="mb-4 p-3 sm:p-4 md:p-5 bg-blue-50 border border-blue-200 rounded-lg">
                <h3 className="text-xs sm:text-sm font-semibold text-blue-900 mb-2 sm:mb-3">Subjects for Selected Class:</h3>
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  {filterSubjects.map(subject => (
                    <span
                      key={subject.id}
                      className="px-2 sm:px-3 md:px-4 py-1 sm:py-1.5 bg-blue-100 text-blue-800 rounded-full text-xs sm:text-sm"
                    >
            ...
            <ResponsiveTableWrapper
                ...
                <table className="w-full border-collapse text-xs sm:text-sm">
                  <thead>
                    <tr className="bg-blue-900 text-white">
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 text-left font-semibold border border-blue-800 text-xs sm:text-sm whitespace-nowrap">Sr.</th>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 text-left font-semibold border border-blue-800 text-xs sm:text-sm whitespace-nowrap">Exam Name</th>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 text-left font-semibold border border-blue-800 text-xs sm:text-sm whitespace-nowrap hidden sm:table-cell">Class</th>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 text-left font-semibold border border-blue-800 text-xs sm:text-sm whitespace-nowrap hidden md:table-cell">Section</th>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 text-left font-semibold border border-blue-800 text-xs sm:text-sm whitespace-nowrap hidden lg:table-cell">Subjects</th>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 text-left font-semibold border border-blue-800 text-xs sm:text-sm whitespace-nowrap">Exam Date</th>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 text-left font-semibold border border-blue-800 text-xs sm:text-sm whitespace-nowrap hidden lg:table-cell">Result Date</th>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 text-left font-semibold border border-blue-800 text-xs sm:text-sm whitespace-nowrap">Status</th>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 text-left font-semibold border border-blue-800 text-xs sm:text-sm whitespace-nowrap">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredExams.length === 0 ? (
                      <tr>
                        <td colSpan="9" className="px-3 py-4 sm:py-6 text-center text-gray-500 border border-gray-200 text-xs sm:text-sm">
                          No exams found
                        </td>
                      </tr>
                    ) : (
                      filteredExams.map((exam, index) => (
                        <tr key={exam.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition`}>
                          ...
                          <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 border border-gray-200 font-medium whitespace-nowrap">{exam.exam_name}</td>
                          <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 border border-gray-200 hidden sm:table-cell whitespace-nowrap">{exam.classes?.class_name || 'N/A'}</td>
                          <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 border border-gray-200 hidden md:table-cell whitespace-nowrap">{exam.sections?.section_name || 'All'}</td>
                          <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 border border-gray-200 max-w-xs truncate hidden lg:table-cell">{exam.subjects}</td>
                          <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 border border-gray-200 whitespace-nowrap">
                            {exam.start_date
                          ...
                          <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 border border-gray-200 whitespace-nowrap hidden lg:table-cell">
              ...
              cardView={
                <CardGrid>
                  {filteredExams.map((exam, index) => (
                    <DataCard key={exam.id}>
                      <CardHeader
                        srNumber={index + 1}
                        name={exam.exam_name}
                        subtitle={`${exam.classes?.class_name || 'N/A'} • ${exam.sections?.section_name || 'All'}`}
                        badge={
                      ...
                      <CardInfoGrid>
                        <CardRow
                          label="Date"
                          value={exam.start_date ? new Date(exam.start_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : 'N/A'}
                        />
                        <CardRow
                          label="Result"
                          value={exam.result_declaration_date ? new Date(exam.result_declaration_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' }) : 'N/A'}
                        />
                        <CardRow
                          label="Status"
                          value={
                            <select
                              ...
                              onChange={(e) => handleStatusChange(exam.id, e.target.value)}
                              disabled={loading}
                              className={`px-2 py-0.5 rounded text-[10px] font-medium border-0 ${
                                exam.status === 'scheduled' ? 'bg-blue-100 text-blue-700' :
                                exam.status === 'ongoing' ? 'bg-green-100 text-green-700' :
                                exam.status === 'completed' ? 'bg-gray-100 text-gray-700' :
                                exam.status === 'cancelled' ? 'bg-red-100 text-red-700' :
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white rounded-lg w-full max-w-[95%] sm:max-w-md md:max-w-lg xl:max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-3 sm:px-4 md:px-6 py-3 sm:py-4 flex justify-between items-center">
            ...
            <div className="p-3 sm:p-4 space-y-3 sm:space-y-4">
              {!editMode && (
                <div className="mb-4 p-3 sm:p-4 md:p-5 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <select
                    ...
                    className="w-full border border-blue-300 rounded px-3 py-2 sm:py-2.5 text-sm sm:text-base bg-white"
                  >
              ...
              <div>
                ...
                <div className="border border-gray-300 rounded p-2 sm:p-3 md:p-4 max-h-60 sm:max-h-72 md:max-h-96 overflow-y-auto">
                  {subjects.length === 0 ? (
                    <p className="text-gray-500 text-xs sm:text-sm">Please select a class first</p>
                  ) : (
                    <div className="space-y-2 sm:space-y-3">
                      {subjects.map(subject => (
                        <div key={subject.id} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-gray-50 rounded">
                          ...
                          {selectedSubjects.includes(subject.id) && (
                            <div className="flex items-center gap-2 ml-6 sm:ml-0">
                              ...
                              <input
                                ...
                                placeholder="100"
                                className="w-16 sm:w-20 md:w-24 border border-gray-300 rounded px-2 py-1 sm:py-1.5 text-xs sm:text-sm"
                                min="1"
                              />
                            </div>
                          )}
            ...
            <div className="sticky bottom-0 bg-gray-50 px-3 sm:px-4 md:px-6 py-3 sm:py-4 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 border-t border-gray-200">
              <button
                onClick={() => {
                  setShowModal(false)
                  resetForm()
                }}
                className="w-full sm:w-auto py-1.5 sm:py-2 px-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 text-xs sm:text-sm"
              >
                Cancel
              </button>
              <button
                ...
                disabled={loading}
                className="w-full sm:w-auto flex items-center justify-center gap-1.5 sm:gap-2 bg-red-500 hover:bg-red-600 text-white py-1.5 sm:py-2 px-3 rounded-lg disabled:opacity-50 text-xs sm:text-sm"
              >
      ...
      {confirmDialog.show && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl z-50 w-[95%] max-w-[95%] sm:max-w-md md:max-w-lg overflow-hidden">
            <div className={`px-3 sm:px-4 md:px-6 py-3 sm:py-4 ${
              confirmDialog.confirmButtonColor === 'red'
                ? 'bg-gradient-to-r from-red-600 to-red-700'
                : 'bg-gradient-to-r from-blue-900 to-blue-800'
            }`}>
              <h3 className="text-sm sm:text-base font-bold text-white">{confirmDialog.title}</h3>
            </div>
            <div className="p-3 sm:p-4 md:p-6">
              <p className="text-xs sm:text-sm text-gray-700 mb-4 sm:mb-6">{confirmDialog.message}</p>
              <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
                <button
                  onClick={hideConfirmDialog}
                  className="w-full sm:w-auto py-1.5 sm:py-2 px-3 text-xs sm:text-sm text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition"
                >
                  Cancel
                </button>
                <button
                  onClick={confirmDialog.onConfirm}
                  className={`w-full sm:w-auto py-1.5 sm:py-2 px-3 text-xs sm:text-sm text-white rounded-lg transition ${
                    confirmDialog.confirmButtonColor === 'red'
