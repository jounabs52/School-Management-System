    ...
    <div className="p-1.5 sm:p-2 md:p-3 lg:p-4 xl:p-6">
              ...
              <table className="w-full border-collapse border border-gray-300 text-xs sm:text-sm">
                <thead>
                  <tr className="bg-blue-900 text-white">
                    ...
                    <th className="border border-gray-300 px-2 sm:px-3 py-1.5 sm:py-2 text-left font-semibold whitespace-nowrap hidden sm:table-cell">Father Name</th>
                    <th className="border border-gray-300 px-2 sm:px-3 py-1.5 sm:py-2 text-center font-semibold whitespace-nowrap hidden md:table-cell">Employee Number</th>
                    <th className="border border-gray-300 px-2 sm:px-3 py-1.5 sm:py-2 text-left font-semibold whitespace-nowrap hidden lg:table-cell">Designation</th>
                  ...
                  {staffList.map((staff, index) => {
                    ...
                    return (
                      <tr key={staff.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        <td className="border border-gray-300 px-2 sm:px-3 py-1.5 sm:py-2 text-gray-800 whitespace-nowrap hidden sm:table-cell">{staff.father_name || ''}</td>
                        <td className="border border-gray-300 px-2 sm:px-3 py-1.5 sm:py-2 text-center text-gray-800 whitespace-nowrap hidden md:table-cell">{staff.employee_number || 'N/A'}</td>
                        <td className="border border-gray-300 px-2 sm:px-3 py-1.5 sm:py-2 text-gray-800 whitespace-nowrap hidden lg:table-cell">{staff.designation || 'N/A'}</td>
                        <td className="border border-gray-300 px-2 sm:px-3 py-1.5 sm:py-2 text-center whitespace-nowrap">
                          <button
                            ...
                            } text-white px-3 sm:px-4 py-1 sm:py-1.5 rounded text-xs sm:text-sm font-medium transition-colors disabled:opacity-90`}
                          >
                    ...
                    return (
                      <DataCard key={staff.id}>
                        <CardHeader
                          ...
                          subtitle={`Father: ${staff.father_name || 'N/A'}`}
                          badge={
                            <button onClick={() => loadStaffDetails(staff)} disabled={isPaid} className={`${isPaid ? 'bg-slate-700 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700'} text-white px-2 py-1 rounded text-[10px] font-medium transition-colors disabled:opacity-90`}>{isPaid ? 'Paid' : 'Pay'}</button>
                          }
                        />
                        <CardInfoGrid>
                          <CardRow label="Emp#" value={staff.employee_number || 'N/A'} />
        ...
        <div className="bg-white rounded-lg shadow-md p-3 sm:p-4 md:p-5 lg:p-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 mb-2 sm:mb-3 pb-2 sm:pb-3 border-b">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-1 sm:gap-2 md:gap-3 text-xs sm:text-sm text-gray-600 mt-1">
                ...
                <span className="hidden md:inline">Designation: {selectedStaff.designation || 'N/A'}</span>
                <span className="hidden lg:inline">|</span>
                <span className="hidden lg:inline">Joining Date: {selectedStaff.joining_date ? new Date(selectedStaff.joining_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : 'N/A'}</span>
              </div>
            </div>
            <button
          ...
          {salaryStructure && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3 mb-2 sm:mb-3">
              <div className="text-center p-2 sm:p-3 bg-blue-50 rounded-lg">
                <div className="text-sm sm:text-base font-bold text-blue-600">{salaryStructure.basic_salary?.toLocaleString() || 0}</div>
                <div className="text-xs sm:text-sm text-gray-600">Basic Salary</div>
              </div>
              <div className="text-center p-2 sm:p-3 bg-green-50 rounded-lg">
                ...
                <div className="text-xs sm:text-sm text-gray-600">Total Allowances</div>
              </div>
              ...
              <div className="text-center p-2 sm:p-3 bg-purple-50 rounded-lg">
                <div className="text-sm sm:text-base font-bold text-purple-600">{salaryStructure.net_salary?.toLocaleString() || 0}</div>
                <div className="text-xs sm:text-sm text-gray-600">Net Salary</div>
              </div>
              <div className="text-center p-2 sm:p-3 bg-orange-50 rounded-lg">
                <div className="text-sm sm:text-base font-bold text-orange-600">{getCurrentYearPaid().toLocaleString()}</div>
                <div className="text-xs sm:text-sm text-gray-600">Paid This Year</div>
              </div>
            </div>
          )}
          ...
          {!salaryStructure && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-2 sm:p-3 mb-2 sm:mb-3">
              <p className="text-yellow-800 font-medium text-xs sm:text-sm">No salary structure defined for this staff member.</p>
              <p className="text-yellow-700 text-xs mt-1">Please create a salary structure before processing payments.</p>
            </div>
          )}

          {/* Action Buttons */}
          <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 mb-2 sm:mb-3">
            <button
              onClick={handleViewSalaryStructure}
              className="flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium bg-red-600 text-white hover:bg-red-700 transition-colors"
            >
          ...
          {salaryStructure && (
            <div className="bg-gray-50 rounded-lg p-3 sm:p-4 md:p-5 lg:p-6">
              <h3 className="text-xs sm:text-sm lg:text-lg font-semibold text-gray-800 mb-2 sm:mb-3">Process Monthly Salary Payment</h3>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 lg:gap-5 mb-2 sm:mb-3">
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Payment Month</label>
                  <select
                    value={paymentMonth}
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Payment Year</label>
                  <select
                    value={paymentYear}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 lg:gap-5 mb-2 sm:mb-3">
                ...
                <div>
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Transaction ID (Optional)</label>
                  <input
                    type="text"
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4 p-2 sm:p-3 bg-blue-50 rounded-lg mb-2">
                <div>
                  {(() => {
                    const existingPayment = paymentHistory.find(
                      p => p.payment_month === paymentMonth && p.payment_year === paymentYear
                ...
                <button
                  ...
                  className={`${
                    paymentStatus === 'pending'
                      ...
                      : 'bg-red-600 hover:bg-red-700'
                  } text-white px-4 py-2 rounded-lg font-semibold text-sm transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed w-full sm:w-auto`}
                >
                  {processingPayment
                    ? 'Processing...'
                    : paymentStatus === 'paid'
      ...
      {showSalaryStructureModal && salaryStructure && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-full sm:max-w-lg lg:max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
            ...
            <div className="p-2 sm:p-3 overflow-y-auto max-h-[calc(95vh-120px)] sm:max-h-[calc(90vh-120px)]">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
              <div className="bg-gradient-to-r from-orange-600 to-orange-700 text-white px-4 py-3 rounded-t-xl">
                <h3 className="text-base sm:text-lg font-bold">No Salary Structure Found</h3>
              </div>
              <div className="p-4 sm:p-6">
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-4 sm:mt-6">
                  <button
                    onClick={() => setShowNoSalaryStructureModal(false)}
                    className="flex-1 px-4 sm:px-3 sm:px-4 py-2 sm:py-3 text-gray-700 font-semibold hover:bg-gray-100 rounded-lg transition border border-gray-300"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={handleNavigateToCreateStructure}
                    className="flex-1 px-4 sm:px-3 sm:px-4 py-2 sm:py-3 bg-blue-600 text-white font-semibold hover:bg-blue-700 rounded-lg transition"
                  >
      ...
      {showPaymentHistoryModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-full sm:max-w-4xl lg:max-w-6xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
            <div className="bg-blue-900 text-white px-3 py-2 flex justify-between items-center">
              <h3 className="text-sm sm:text-base font-bold">Payment History - {selectedStaff.first_name} {selectedStaff.last_name}</h3>
              <button
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
              <div className="bg-gradient-to-r from-yellow-600 to-yellow-700 text-white px-3 py-2 rounded-t-xl">
                <h3 className="text-base sm:text-lg font-bold">Confirm Duplicate Payment</h3>
              </div>
              <div className="p-3 sm:p-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                  ...
                  <button
                    ...
                    disabled={processingPayment}
                    className="flex-1 px-4 sm:px-3 sm:px-4 py-2 sm:py-3 bg-green-600 text-white font-semibold hover:bg-green-700 rounded-lg transition disabled:opacity-50"
                  >
      ...
      {showDeletePaymentModal && (
        <ModalOverlay onClose={() => !deleting && setShowDeletePaymentModal(false)}>
