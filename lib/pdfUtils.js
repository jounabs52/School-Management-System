 ...
 * @param {Object} options - Additional options (subtitle, info, session, pdfSettings, etc.)
...
export const addPDFHeader = (doc, schoolData, title, options = {}, startY = 10) => {
  ...
  // Get PDF settings (use passed settings or defaults)
  const pdfSettings = options.pdfSettings || {}

  // Convert hex to RGB helper
  const hexToRgb = (hex) => {
    if (!hex) return PDF_COLORS.primary
  ...
  // Use settings colors or fallback to defaults
  const headerBgColor = pdfSettings.headerBackgroundColor ? hexToRgb(pdfSettings.headerBackgroundColor) : PDF_COLORS.primary
  ...
  // Generated date (bottom right of header) - respect settings
 ...
 */
export const addPDFFooter = (doc, pageNumber = 1, totalPages = 1, pdfSettings = {}) => {
  const pageWidth = doc.internal.pageSize.getWidth()
  const pageHeight = doc.internal.pageSize.getHeight()

  // CRITICAL: Use absolute positioning from page bottom
  // For A4: pageHeight = 297mm, so footer will be at 287mm (10mm from bottom)
  const absoluteFooterY = pageHeight - 10
  const absoluteFooterLineY = pageHeight - 15

  // Save the current graphics state to avoid affecting content rendering
  doc.saveGraphicsState()

  // Draw footer line at absolute position from page bottom
  doc.setDrawColor(...PDF_COLORS.border)
  doc.setLineWidth(0.3)
  doc.line(15, absoluteFooterLineY, pageWidth - 15, absoluteFooterLineY)
  ...
  // Left: Footer text from settings (if provided)
  if (pdfSettings.footerText) {
    doc.text(pdfSettings.footerText, 15, absoluteFooterY)
  }

  // Center: Date & Time (respect includeDate setting, default: shown)
  if (pdfSettings.includeDate !== false) {
    const now = new Date()
    const dateStr = now.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
    const timeStr = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })
    doc.text(`${dateStr} ${timeStr}`, pageWidth / 2, absoluteFooterY, { align: 'center' })
  }

  // Right: Page number (respect includePageNumbers setting, default: shown)
  if (pdfSettings.includePageNumbers !== false) {
    doc.text(`Page ${pageNumber} of ${totalPages}`, pageWidth - 15, absoluteFooterY, { align: 'right' })
  }

  // Restore graphics state to not affect subsequent rendering
